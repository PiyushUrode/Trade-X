...
import { AdminBanksdata } from '../../Dashboard/Contentpages/Dummydata.js'; // Updated to import AdminBanks
...
const AdminBanks = () => {
  ...
  // Dummy data
  const [data, setData] = useState(AdminBanksdata); // Using dummy data
        ...
        <table className="min-w-full bg-black text-white text-left border-collapse">
          <thead>
            <tr>
              ...
              <th className="border-b p-4 cursor-pointer hover:bg-black" onClick={() => handleSort('accountHolder')}>
                Account Holder Name
              </th>
              <th className="border-b p-4 cursor-pointer hover:bg-black" onClick={() => handleSort('bankName')}>
                Bank Name
              </th>
              <th className="border-b p-4 cursor-pointer hover:bg-black" onClick={() => handleSort('country')}>
                Country
              </th>
              <th className="border-b p-4 cursor-pointer hover:bg-black" onClick={() => handleSort('swiftCode')}>
                Swift Code
              </th>
          ...
          <tbody>
            {currentItems.map((item) => (
              <tr key={item.id} className="hover:bg-slate-500">
                <td className="border-b p-4"><img src={item.logo} alt={item.accountHolder} className="h-8 w-8" /></td>
                <td className="border-b p-4">{item.accountHolder}</td>
                <td className="border-b p-4">{item.bankName}</td>
                <td className="border-b p-4">{item.country}</td>
                <td className="border-b p-4">{item.swiftCode}</td>
