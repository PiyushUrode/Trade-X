...
import { Coinnetworksdata } from '../Dummydata.js'; // Importing dummy data
...
const CoinNetwork = () => {
  ...
  // Dummy data
  const [data, setData] = useState(Coinnetworksdata); // Using dummy data
        ...
        <table className="min-w-full bg-black text-white text-left border-collapse">
          <thead>
            <tr>
              ...
              <th className="border-b p-4 cursor-pointer hover:bg-black" onClick={() => handleSort('currency')}>
                Currency
              </th>
              <th className="border-b p-4 cursor-pointer hover:bg-black" onClick={() => handleSort('type')}>
                Type
              </th>
              <th className="border-b  p-4 cursor-pointer hover:bg-black" onClick={() => handleSort('network')}>
                Network
              </th>
              <th className="border-b p-4 cursor-pointer hover:bg-black" onClick={() => handleSort('status')}>
                Status
              </th>
              <th className="border-b p-4 cursor-pointer hover:bg-black" onClick={() => handleSort('createdAt')}>
                Created At
              </th>
              <th className="border-b p-4 cursor-pointer hover:bg-black" onClick={() => handleSort('updatedAt')}>
                Updated At
              </th>
          ...
          <tbody>
            {currentItems.map((item) => (
              <tr key={item.id} className="hover:bg-slate-500">
                <td className="border-b p-4"><img src={item.logo} alt={item.currency} className="h-8 w-8" /></td>
                <td className="border-b p-4">{item.currency}</td>
                <td className="border-b p-4">{item.type}</td>
                <td className="border-b p-4">{item.network}</td>
                ...
                <td className="border-b p-4">{item.updatedAt}</td>
                <td className="border-b p-4">
